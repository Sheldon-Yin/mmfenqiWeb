            ...
            return {
                ...
                getPrice: function (key, data) {
                    ...
                    if (items.length === keys.length) {
                        key_account_Map[key] = data[key] ? data[key].presentPrice : null;
                    ...
                    for (i = 0; i < keys.length; i++) {
                        ...
                        if (j < keys[i].length && items.length > 0) {
                            ...
                            for (m = 0; m < keys[i].length; m++) {
                                var newPrice = this.getPrice(n.concat(keys[i][m], items).join(skuConfig.splitStr), data);
        ...
        .directive('uiSku', ['$log', 'skuConfig', 'utilService', function ($log, skuConfig, utilService) {
            return {
                ...
                link: function (scope, element, attrs, ctrls, transclude) {
                    ...
                    scope.keyMap = {};
                    scope.selected = [];
                    scope.skuData.$promise.then(function (response) {
                        scope.skuDataGet = response.data.goodsDetailsResponse.goodsCombinationExtMap;
                        console.log(scope.skuDataGet);
                        scope.keys = utilService.getKeys(scope.skuDataGet);
                    ...
                    scope.onSelect = function (key) {
                        ...
                        scope.onOk({price: utilService.getPrice(check.join(skuConfig.splitStr), scope.skuData),
                            selectedNum: check.length,
                            storeId: utilService.getStoreId(check.join(skuConfig.splitStr), scope.skuData)});
                    };
